...
def rand_fourier_features(X, D):
    # d: dimension of data; D: dimension of samples
    d = X.shape[1]
    w = np.random.multivariate_normal(mean = np.zeros(d), cov = np.eye(d), size = D) #TODO: find correct mean and cov
    b = np.random.uniform(low = 0, high = 2*np.pi, size = D)
    z = np.sqrt(2/D) * np.cos(np.matmul(w, x) + b)
    return z
